...
export default async (req) => {
  ...
  // userId é obrigatório: sem ele não dá pra checar/debitar crédito nenhum.
  // Antes, uma chamada direta ao endpoint sem userId pulava a checagem de
  // crédito inteira e conversava com a Claude de graça, sem limite.
  if (!userId) {
    return json({ error: { message: 'userId obrigatório.' } }, 400);
  }
  ...
  const creditRecord = existing?.data || { credits: FREE_TRIAL_CREDITS };
  const creditEtag = existing?.etag || null;
