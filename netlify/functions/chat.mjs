...
export default async (req) => {
  ...
  const creditsStore = userId ? getStore('lumi-credits') : null;
  let creditRecord = null;
  let creditEtag = null;

  if (creditsStore) {
    const existing = await creditsStore.getWithMetadata(userId, { type: 'json' });
  ...
  if (status >= 200 && status < 300 && creditsStore) {
    ...
    const result = await creditsStore.setJSON(userId, updated, writeOpts).catch(() => null);
    if (!result || !result.modified) {
      ...
      const fresh = (await creditsStore.get(userId, { type: 'json' })) || { credits: FREE_TRIAL_CREDITS };
      fresh.credits = Math.max(0, fresh.credits - 1);
      await creditsStore.setJSON(userId, fresh).catch(() => {});
    }
  }
